...
you keep if the manager changes. See [Why Paperful](../start/why.md).
...
are already in Zotero. Stance: [quiet-mirror.md](../paths/quiet-mirror.md). House
...
- `paperful doctor` — preflight. Colours: **green** = ready; **amber** = usable with
  ...
  See [commands](../reference/commands.md#doctor).
...
- `paperful dedupe` / `paperful gaps` — duplicate packs and PDF/DOI counts.
  ...
  (title+year also needs `--apply-medium`). See [dedupe](../howto/dedupe.md).
...
closes the pack it opened. See [Workflows](../howto/workflows.md).
...
- [ezproxy.md](../howto/ezproxy.md) / [sessions.md](../howto/sessions.md) — campus proxy and browser vault
- [docker.md](../howto/docker.md) — build-local image (host Zotero + headed login stay outside)
- [zotero.md](../howto/zotero.md) — local API, write keys, attachment modes, ghosts
- [mendeley.md](../howto/mendeley.md) — REST, OAuth, annotations as notes (seeking testers)
- [endnote.md](../howto/endnote.md) — SQLite read, XML import bundle (seeking testers)
