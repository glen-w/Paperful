...
Vendor-by-vendor notes live in the [comparison reference](../reference/comparison-reference.md).
...
| If you need… | Look at… |
|--------------|----------|
| A platform-agnostic **mirror** of the library (backup, and a way out if the citation manager changes). **Zotero is well tested.** Mendeley and EndNote adapters are seeking testers | **Paperful** `snapshot` / `restore` — [quiet mirror](../paths/quiet-mirror.md). Do not treat the other adapters as proven |
...
| **Grow the library** from a keyword or a DOI’s references, then optionally fill PDFs | **Paperful snowball** — dry-run, `--gate auto`, and `--fetch-pdfs` are in the tree ([snowball](../paths/snowball.md)). ORCID and in-app one-hop browsers stay separate ([zotero-snowball](https://github.com/socratic-irony/zotero-snowball), [Citegeist](https://github.com/phdemotions/zotero-citegeist)). General harvesters without the mirror: [findpapers](https://github.com/jonatasgrosman/findpapers), [opencite](https://github.com/neuromechanist/opencite) |
...
More branches: [comparison reference](../reference/comparison-reference.md#choosing-in-one-glance).

## Related docs

- [architecture.md](architecture.md) — disk-first adapters, circuit breaker, Sci-Hub
- [ROADMAP.md](../contribute/ROADMAP.md) — mirror contract toward 1.0; optional LLM; snowball phases
- [snowball.md](../paths/snowball.md) — planned keyword / DOI / ORCID library building
- [comparison-reference.md](../reference/comparison-reference.md) — vendor notes and extra tables
- [commands.md](../reference/commands.md) — CLI and disk artifacts
- [config.md](../reference/config.md) — `config.toml` and operations
