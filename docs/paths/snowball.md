# Path: snowball discovery

**Who:** you are growing a collection from a keyword, DOI neighbourhood,
ORCID, or seed set — not only filling what is already there.  
**Goal:** review candidates, create parents under an explicit gate, optionally
fill PDFs.

Finish [Quick start](../start/quickstart.md) first. Enable snowball in
`config.toml`:
...
[ROADMAP](../contribute/ROADMAP.md#snowball).

## Path

1. Dry-run a seed (no library writes):

```sh
docker compose run --rm paperful snowball search "area based management tools" --year-from 2018
# or: snowball doi … / snowball orcid … / snowball collection "Inbox/Seeds"
```

2. Inspect `state/snowball/<run-id>/` (candidates + summary).
3. Keep rows (`keep = true`) and apply under a writing gate, **or** use
   `gate = auto` only after you trust that job:

```sh
docker compose run --rm paperful snowball apply <run-id> --collection "Inbox/Snowball"
```

4. Optional PDFs for keys just created (`fetch_pdfs`), or run
   [Fill missing PDFs](fill-pdfs.md) on the target collection afterward.
5. Save a repeatable profile after a good dry-run
   (`snowball run --profile …`).

`paperful run` and `paperful all` refuse snowball profiles — keep discovery
jobs separate from fetch profiles.
...
gate. [`run`](../reference/commands.md) fills PDFs for items that already exist. With
`fetch_pdfs`, snowball calls that same `run` in the same process, limited to
the keys it just created.
...
Precedence matches [run profiles](../reference/config.md#run-configs-profiles): built-in
...
| [paperscraper](https://github.com/jannisborn/paperscraper) | — | DOI-list PDFs when you have no Zotero. See [comparison](../explain/comparison.md) |
...
- [ROADMAP](../contribute/ROADMAP.md#snowball) — phases
- [comparison](../explain/comparison.md) — paperscraper, findpapers, in-Zotero plugins
- [config](../reference/config.md#run-configs-profiles) — profile precedence this lane reuses
- [commands](../reference/commands.md) — `run`, which `fetch_pdfs` calls
