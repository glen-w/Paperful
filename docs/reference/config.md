...
and [architecture](../explain/architecture.md).
...
| `[mendeley].client_id` / `client_secret` | `""` | Elsevier OAuth app from [dev.mendeley.com/myapps.html](https://dev.mendeley.com/myapps.html). Or `PAPERFUL_MENDELEY_CLIENT_*`. See [Mendeley](../howto/mendeley.md) |
| `[mendeley].redirect_uri` | `http://127.0.0.1:8765/callback` | Must match the app. Host-only (`session login mendeley`) |
| `[endnote].library` | (none) | Path to the `.enl` file. Matching `.Data` (with `sdb/sdb.eni`) must sit beside it. See [EndNote](../howto/endnote.md) |
...
| `ezproxy_base` | `""` (disabled) | Campus proxy prefix ending in `url=` — see [Campus EZProxy](../howto/ezproxy.md) |
...
you add `"scihub"` to `sources` or pass `--scihub` — see [Sci-Hub](../howto/scihub.md).
...
[pack](commands.md). Full recipes: [Workflows](../howto/workflows.md).
...
[architecture § Grey literature](../explain/architecture.md#grey-literature) for hosts
and symbol patterns. If you use the optional Docker image, put packs next to
config under `/data` (see [Docker](../howto/docker.md)).
...
walkthrough, model advice, Docker networking, and troubleshooting: [LLM](../howto/llm.md).
