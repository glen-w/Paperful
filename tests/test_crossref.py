...
def _works(*items):
    return httpx.Response(
        200, content=json.dumps({"message": {"items": list(items)}}).encode()
    )
...
def test_weak_match_and_year_mismatch_rejected():
    client = mock_client(
        lambda r: _works(
            _work("10.1/x", "Something completely different about fisheries", 2019)
        )
    )
    assert (
        crossref_lookup(client, "Marine genetic resources beyond national jurisdiction")
        is None
    )
    client2 = mock_client(
        lambda r: _works(
            _work(
                "10.1/x", "Marine genetic resources beyond national jurisdiction", 2005
            )
        )
    )
...
def test_short_title_retry_when_subtitle_glued_on():
    ...
    def handler(req):
        ...
        return _works(
            _work("10.1/short", "Marine genetic resources beyond national jurisdiction")
        )
    ...
    assert (
        len(calls) == 2
        and calls[1] == "Marine genetic resources beyond national jurisdiction"
    )


def test_short_title_heuristics():
    assert (
        short_title("Main title here please: and a subtitle")
        == "Main title here please"
    )
    assert short_title("Too short: subtitle") is None
    assert (
        short_title("Version 2.0 of the plan is here")
        == "Version 2.0 of the plan is here"
    )
...
def test_too_short_title_and_api_errors_return_none():
    ...
    assert (
        crossref_lookup(client, "A perfectly reasonable title of some length") is None
    )
    bad_json = mock_client(lambda r: httpx.Response(200, content=b"<html>"))
    assert (
        crossref_lookup(bad_json, "A perfectly reasonable title of some length") is None
    )
