...
def test_nested_tables_parse(tmp_path):
    raw = {
        "llm": {"enabled": True, "provider": "litellm", "model": "gpt-x", "api_base": "https://x"},
    ...
    assert cfg.llm_enabled and cfg.llm_provider == "litellm" and cfg.llm_model == "gpt-x"
...
def test_llm_title_gated_off(llm_cfg, monkeypatch):
    ...
    assert llm_title.propose_llm_title(llm_cfg, make_item(), {"title_all_caps"}, None, None) is None


def test_llm_title_requires_title_code(llm_cfg, monkeypatch):
    llm_cfg.fix_metadata_llm_title = True
    _ground(monkeypatch, llm_title)
    monkeypatch.setattr(llm_title, "get_client", lambda cfg: StubLLM(data={"title": "X"}))
    assert llm_title.propose_llm_title(llm_cfg, make_item(), {"missing_doi"}, None, None) is None
...
def test_llm_title_rejects_ungrounded(llm_cfg, monkeypatch):
    ...
    assert llm_title.propose_llm_title(llm_cfg, item, {"title_all_caps"}, None, None) is None
...
def test_llm_title_continues_on_failure(llm_cfg, monkeypatch):
    ...
    assert llm_title.propose_llm_title(llm_cfg, make_item(), {"title_html"}, None, None) is None
...
def test_collect_patches_includes_llm_title(llm_cfg, monkeypatch):
    ...
    _ground(monkeypatch, llm_title)
    monkeypatch.setattr(
        llm_title, "get_client", lambda cfg: StubLLM(data={"title": "Marine governance tools"})
    )
...
def test_pdf_match_mismatch_yields_finding(llm_cfg, monkeypatch):
    ...
    _ground(monkeypatch, llm_pdf_match)
    stub = StubLLM(data={"match": False, "confidence": 0.91, "reason": "different authors"})
...
def test_pdf_match_ok_yields_none(llm_cfg, monkeypatch):
    ...
    _ground(monkeypatch, llm_pdf_match)
    monkeypatch.setattr(
        llm_pdf_match, "get_client", lambda cfg: StubLLM(data={"match": True, "confidence": 0.95})
    )
    assert llm_pdf_match.pdf_identity_finding(llm_cfg, make_item(), None, None) is None
...
def test_pdf_match_low_confidence_match_flags(llm_cfg, monkeypatch):
    ...
    _ground(monkeypatch, llm_pdf_match)
    monkeypatch.setattr(
        llm_pdf_match, "get_client", lambda cfg: StubLLM(data={"match": True, "confidence": 0.3})
    )
    f = llm_pdf_match.pdf_identity_finding(llm_cfg, make_item(), None, None)
...
def test_pdf_match_no_text_is_silent(llm_cfg, monkeypatch):
    ...
    monkeypatch.setattr(llm_pdf_match, "get_client", lambda cfg: StubLLM(data={"match": False}))
    assert llm_pdf_match.pdf_identity_finding(llm_cfg, make_item(), None, None) is None
...
def test_lint_item_appends_identity_finding(llm_cfg, monkeypatch):
    ...
    _ground(monkeypatch, llm_pdf_match)
    monkeypatch.setattr(
        llm_pdf_match, "get_client", lambda cfg: StubLLM(data={"match": False, "reason": "nope"})
    )
    findings = lint_item(mock_client(lambda r: httpx.Response(500)), llm_cfg, make_item())
...
class NoteBackend:
    ...
    def find_child_note_keys(self, item_key, tag):
        return [k for k, (parent, t) in self.notes.items() if parent == item_key and t == tag]
...
def test_summarize_refuses_when_identity_flagged(llm_cfg, monkeypatch):
    ...
    _ground(monkeypatch, llm_pdf_match)
    monkeypatch.setattr(
        llm_pdf_match, "get_client", lambda cfg: StubLLM(data={"match": False, "reason": "wrong"})
    )
    monkeypatch.setattr(summarize, "get_client", lambda cfg: StubLLM(text="<p>s</p>"))
    item = make_item(has_pdf=True)
...
def test_summarize_remote_footer(llm_cfg, monkeypatch):
    ...
    _ground(monkeypatch, summarize)
    monkeypatch.setattr(summarize, "get_client", lambda cfg: StubLLM(text="<p>s</p>"))
    html = summarize.summarize_item(llm_cfg, make_item(has_pdf=True), None, None).read_text()
