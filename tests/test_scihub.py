...
from paperful.sources.scihub import (
    extract_altcha_urls,
    is_captcha_page,
    parse_page,
    solve_altcha,
...
def test_parse_found_page_gives_absolute_pdf_url():
    ...
    assert (
        res.pdf_url
        == "https://sci-hub.ru/storage/zero/6364/41ab8ffb30f3383b52e955640fd17dc4/ding2017.pdf"
    )
...
def test_parse_captcha_page():
    ...
    assert extract_altcha_urls(html) == (
        "/captcha/challenge/91226685",
        "/captcha/solution/91226685",
    )
...
def test_parse_unknown_layout_is_error_not_not_found():
    res = parse_page(
        "<html><head><title>Sci-Hub</title></head><body>hello</body></html>", BASE
    )
...
def test_parse_not_in_database_layout():
    res = parse_page(
        (FIX / "scihub_not_in_db.html").read_text(),
        "https://sci-hub.ru/10.18623/rvd.v14i28.1038",
    )
    assert res.outcome is Outcome.NOT_FOUND
